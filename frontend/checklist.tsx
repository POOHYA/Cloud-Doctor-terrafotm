...
const [selectedServices, setSelectedServices] = useState<string[]>(services);
    const [answers, setAnswers] = useState<Record<string, boolean>>({});
        ...
        <div className="p-6 max-w-5xl mx-auto">
            <h1 className="text-2xl font-bold mb-4">AWS 보안 체크리스트</h1>

            <div className="flex flex-wrap gap-2 mb-6">
                {services.map((service) => (
                <button key={service} className={`px-4 py-2 rounded ${ selectedServices.includes(service)
                    ? "bg-blue-500 text-white" : "bg-gray-200" }`} onClick={()=> toggleService(service)}
                    >
                    {service}
                </button>
                ))}
            ...
            <table className="table-auto w-full border-collapse border border-gray-300">
                <thead>
                    <tr className="bg-gray-100">
                        ...
                        <th className="border border-gray-300 p-2">체크</th>
                    </tr>
                </thead>
                <tbody>
                    ...
                    <tr key={item.id} className="hover:bg-gray-50">
                        ...
                        <td className="border border-gray-300 p-2 text-center">
                            <input type="checkbox" checked={!!answers[item.id]} onChange={()=> handleCheck(item.id)}
                            />
                        </td>
                    </tr>
                    ))}
